		...
		if (allowNether && allowEnd) {
			...
			if (!allowNether) {
				...
				getLogger().info(ChatColor.DARK_RED
						+ "Ã„ndere den Wert in der 'server.properties' auf 'true' und starte den Server neu!");
			}
			if (!allowEnd) {
				getLogger().info(ChatColor.DARK_RED + "'allow-end' ist 'false'");
				getLogger().info(ChatColor.DARK_RED
						+ "Ã„ndere den Wert in der 'bukkit.yml' auf 'true' und starte den Server neu!");
			}
			getLogger().info(ChatColor.DARK_RED + "Server fÃ¤hrt herunter...");
			return true;
		}
	}
